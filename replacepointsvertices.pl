...
for (my $tx = 0, my $row = 0; $tx < @newtextures; $tx += 7, $row += 3) {
	my $p1 = "$newtextures[$tx+1] $newtextures[$tx+2]\n";
	my $p2 = "$newtextures[$tx+3] $newtextures[$tx+4]\n";
	my $p3 = "$newtextures[$tx+5] $newtextures[$tx+6]\n";
	if (!$tciset{$p1}) {
		$tciset{$p1} = $row;
	}
	if (!$tciset{$p2}) {
	...
	$uv .= $p1.$p2.$p3;
	$tci .= ($tciset{$p1})." ".($tciset{$p2})." ".($tciset{$p3})." -1\n";
}
