...
#echo "perl toddlerHRE.pl Toddler_ < ${PREFIX}.gltfholger.x3dv > ${PREFIX}toddlerized1.x3dv"
#perl toddlerHRE.pl Toddler_ < ${PREFIX}.gltfholger.x3dv > "${PREFIX}"toddlerized1.x3dv
...
echo "perl toddlerHRE.pl Toddler_ ${PREFIX}.gltfmichalis.x3dv < ${PREFIX}.gltfmichalis.x3dv > ${PREFIX}toddlerized1.x3dv"
...
#echo "perl reformatCenters.pl < ${INPUTDIR}/JointCenterCheck.txt > ${PROCESSDIR}/7_3 joint location.txt"
#perl reformatCenters.pl < "${INPUTDIR}"/JointCenterCheck.txt > "${PROCESSDIR}/7_3 joint location.txt"

echo "perl Newcenters.pl ${INPUTDIR}/7_3 joint location.txt ${PREFIX}swept.x3dv < ${PREFIX}swept.x3dv > ${PREFIX}centered.x3dv"
...
#echo "perl moveSkin.pl ${PREFIX}swept.x3dv < ${PREFIX}swept.x3dv > ${PREFIX}skinplaced.x3dv"
#perl moveSkin.pl "${PREFIX}"swept.x3dv < "${PREFIX}"swept.x3dv > "${PREFIX}"skinplaced.x3dv
...
echo "perl patchupLily.pl < ${PREFIX}imaged.x3dv > ${PREFIX}final.x3dv"
